...
import type { MatchOutcomeInsight } from "./types";
import { isWatchOutInsight, renderInsightText } from "./utils";

export function MatchOutcomeSignalsAccordion({
	insights,
}: {
	insights: MatchOutcomeInsight[];
}) {
	const supportingInsights = insights.filter((insight) => !isWatchOutInsight(insight));
	const watchOutInsights = insights.filter((insight) => isWatchOutInsight(insight));
